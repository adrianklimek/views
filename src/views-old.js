...
(function(window, factory) {
    ...
    // Select correct animation event for a browser
    function selectAnimationEvent() {
        var element = document.createElement('div');
        if (typeof element.style.WebkitAnimation != 'undefined') return 'webkitAnimationEnd';
        if (typeof element.style.OAnimation != 'undefined') return 'oAnimationEnd';
        if (typeof element.style.animation != 'undefined') return 'animationend';
        return false;
    }
    ...
    Views.prototype.init = function() {
        ...
        this.css = {
            view: 'position:fixed;top:0;left:0;width:100%;height:100%;background-color:rgba(0,0,0,.75);padding:50px 15px;box-sizing:border-box;cursor:pointer;',
        ...
        // Select animation event for browser
        this.eventName = selectAnimationEvent();
    ...
    Views.prototype.open = function() {
        ...
        if (this.options.openAnimation && this.eventName !== false) {
            this.view.addEventListener(this.eventName, this, false);
    ...
    Views.prototype.close = function() {
        ...
        if (!this.options.closeAnimation || this.eventName === false) {
            this.remove();
        }
    };
    ...
    Views.prototype.onanimationEnd = function(event) {
        if (event.currentTarget.classList.contains(this.names.class.opening)) {
            this.view.classList.remove(this.names.class.opening);
        }
        else if (event.currentTarget.classList.contains(this.names.class.closing)) {
            this.remove();
        }
    };
